...
conf.set('spark.hadoop.fs.s3a.access.key', 'AKIA3PU7RNER7CQGJVLE')    #AWS Access Key
conf.set('spark.hadoop.fs.s3a.secret.key', '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')   #AWS Secret Key
...
host_ = 'jdbc:mysql://' + 'henry-datalake-db.cxlhah81ocl3.ap-northeast-2.rds.amazonaws.com/newyork_taxi'
user_ = 'admin'
...
#             .option('url', host_).option('driver', 'com.mysql.cj.jdbc.Driver') \
#             .option('dbtable', write_table).option('user', user_).option('password', password_).save()
